...
import { Monitor, Gamepad2, Crown, Users, Tv, Speaker, Refrigerator } from "lucide-react"
...
export function SeatSelection({ section, selectedSeats, onSelectSeat }: SeatSelectionProps) {
  // Group seats by row
    ...
    <div className="space-y-6">
      ...
      {isVIPRoom && (
        <div className="p-4 rounded-xl bg-yellow-500/5 border border-yellow-500/20">
          <h4 className="text-sm font-medium text-yellow-400 mb-3">Room Features</h4>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              ...
              <span>{section.roomCapacity === 10 ? "4K Projector" : "4K Display"}</span>
            </div>
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
      ...
      <div className="flex flex-wrap items-center gap-4 text-xs">
        ...
        <div className="flex items-center gap-2">
          <div className={cn(
            ...
            section.type === "pc" && "bg-cyan-500/30 border-cyan-500 shadow-[0_0_10px_rgba(0,212,255,0.5)]",
            section.type === "console" && "bg-green-500/30 border-green-500 shadow-[0_0_10px_rgba(34,197,94,0.5)]",
            section.type === "ps5" && "bg-blue-500/30 border-blue-500 shadow-[0_0_10px_rgba(59,130,246,0.5)]",
            section.type === "vip" && "bg-yellow-500/30 border-yellow-500 shadow-[0_0_10px_rgba(234,179,8,0.5)]"
          )} />
          ...
          <div className="flex justify-center mb-6">
            <div className="flex items-center gap-2 px-4 py-2 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
              ...
              <span className="text-sm text-yellow-400 font-medium">
                {section.roomCapacity}-Seat {section.roomCapacity === 5 ? "Private" : "Party"} Room
              </span>
            </div>
          </div>
        )}
                ...
                {rows[rowKey].map((seat) => {
                  ...
                  return (
                    <button
                      ...
                      title={`${seat.row}${seat.number} - ${seat.specs || seatTypeLabels[section.type]}`}
                      className={cn(
                        "relative w-14 h-14 rounded-lg flex flex-col items-center justify-center",
                        "transition-all duration-200 border",
                        isVIPRoom && "w-16 h-16", // Slightly larger for VIP
                        isSelected && section.type === "pc" && "bg-cyan-500/30 border-cyan-500 shadow-[0_0_15px_rgba(0,212,255,0.5)] scale-105",
                        isSelected && section.type === "console" && "bg-green-500/30 border-green-500 shadow-[0_0_15px_rgba(34,197,94,0.5)] scale-105",
                    ...
                    >
                      <Icon className={cn(
                        "w-5 h-5",
                        isVIPRoom && "w-6 h-6",
                        isSelected && section.type === "pc" && "text-cyan-400",
                        isSelected && section.type === "console" && "text-green-400",
      ...
      {section.seats[0]?.specs && (
        <div className={cn(
          ...
          <p className="text-xs text-muted-foreground">
            ...
            )}>Specs:</span> {section.seats[0].specs}
          </p>
        </div>
      )}
    </div>
